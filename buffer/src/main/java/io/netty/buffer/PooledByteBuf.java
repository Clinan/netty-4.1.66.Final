...
abstract class PooledByteBuf<T> extends AbstractReferenceCountedByteBuf {
    ...
    // 直接内存的是一个容量为16MB的directByteBuffer。
    protected T memory;
    ...
    /**
     * 在memory中，duplicate一块共享内存。作为TmpNioBuf, TmpNioBuf已存在的情况下，会被清理
     * @return
     */
     ...
     * 新缓冲区的容量、限制、位置和标记值将与此缓冲区的相同。 当且仅当此缓冲区为直接缓冲区时，新缓冲区为直接缓冲区，且仅当此缓冲区为只读时，新缓冲区为只读缓冲区。
     * @param memory DirectByteBuffer
     * @return
     */
    ...
    public final int setBytes(int index, ScatteringByteChannel in, int length) throws IOException {
        try {
            // 调用jdk的方法，将数据导入到变量memory中。internalNioBuffer返回的是memory.duplicate()的DirectByteBuffer
