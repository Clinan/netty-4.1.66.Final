...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    public ChannelHandlerContext fireChannelRead(final Object msg) {
        // 找到持有InboundChannelHandler的ChannelHandlerContext
        invokeChannelRead(findContextInbound(MASK_CHANNEL_READ), msg);
        return this;
    }

    static void invokeChannelRead(final AbstractChannelHandlerContext next, Object msg) {
        // 判断实现了引用次数的类，如果实现了，那就touch一下，方便调试 DEBUG
        final Object m = next.pipeline.touch(ObjectUtil.checkNotNull(msg, "msg"), next);
        // 注意，这里这句话的意思是拿着ChannelHandlerContext持有的线程（NioEventLoop）去执行，这个对应的Handler。
        // 也就是Worker线程。在创建NioSocketChannel。设置了这个EventLoop的。
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {
            // 第一次进入是ChannelPipeline
            // 如果reactor模式是共享的话，则直接执行。
    ...
    private void invokeChannelReadComplete() {
        if (invokeHandler()) {
            try {
                // 如果ChannelHandler是ChannelPipeline的话，
    ...
    private static boolean skipContext(
        ...
        return (ctx.executionMask & (onlyMask | mask)) == 0 ||//1 1111 1110      0010 0000
